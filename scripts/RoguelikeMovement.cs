...
public partial class RoguelikeMovement : Node2D {
	...
	private int totalGoals = 4;
	private int goalsCleared = 0;
	...
	public override void _Ready(){
		...
		var goalArea = GetNode<Area2D>("Treats/DummyGoal");             // This becomes the collision for the canna-biscuits that end a sucessful round
		var goalArea1 = GetNode<Area2D>("Treats/DummyGoal2");
		var goalArea2 = GetNode<Area2D>("Treats/DummyGoal3");
		var goalArea3 = GetNode<Area2D>("Treats/DummyGoal4");
		...
		goalArea1.BodyEntered += (body) => {
			if (body == activeDog)
				goalArea1.QueueFree();
				OnGoalCleared();
		};
		
		goalArea2.BodyEntered += (body) => {
			if (body == activeDog)
				goalArea2.QueueFree();
				OnGoalCleared();
		};
		
		goalArea3.BodyEntered += (body) => {
			if (body == activeDog)
				goalArea3.QueueFree();
				OnGoalCleared();
		};
